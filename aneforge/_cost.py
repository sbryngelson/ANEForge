...
def _constants() -> dict:
  ...
  if path is not None:
    try:
      ...
      # derive COMPUTE from the matmul + conv scaling fits (us/flop -> flop/us)
      mm = model.get("matmul", {})
      ...
      # cut penalty: smallest measured bridge min as the marginal cut cost.
...
def _cost_model_path():
  ...
  for up in Path(__file__).resolve().parents:        # legacy out-of-tree
...
_M1_FIT_BW_BYTES = 9.0e9            # effective streaming BW (jointly fit)
_M1_FIT_OVERHEAD_US = 220.0         # additive per-program dispatch overhead (0.22 ms)
_M1_MEASURED_PEAK_TFLOPS = 1.8      # headline fp16 peak (project_peak absolute anchor)
_CLOCK_FRACTION = 0.8              # operating clock ~= 0.8 * fmax

# arch -> per-family fallback curve key for chips without their own entry.
...
def _curve_for_arch(arch: str) -> dict:
  """Resolve an arch string to its cost curve (case-insensitive, per-family fallback)."""
  curves = _curves()
  key = arch.strip()
  # exact, then case-insensitive (JSON uses 'H13'; arch is 'h13').
  if key in curves: return curves[key]
  low = {k.lower(): k for k in curves}
  if key.lower() in low: return curves[low[key.lower()]]
  # fall back to the family's representative die.
  from . import _targets as _TG
  fam = _TG.family_of_arch(key)                  # raises on an unknown name
...
def _interp(x: float, xs, ys) -> float:
  """Piecewise-linear interp of ys at x over sorted xs (clamped)."""
...
def _eff_freq_at_op(curve: dict) -> tuple[float, float]:
  """(operating_freq, effective_freq) at f = 0.8*fmax (eff_freq interpolated from eff_map_0x7a8)."""
...
# Effective BW tracks CORE COUNT, not clock; `util` is h14's mid-utilization compute ramp.
_ANCHORS = {
  "h13": {"bw": _M1_FIT_BW_BYTES, "floor_us": _M1_FIT_OVERHEAD_US, "peak": _M1_FIT_PEAK_FLOPS},
  # eff_peak = peak * min(1, (flops/F)^q): a CAPPED power law.
...
def _anchor_for_arch(arch: str) -> str:
  """The measured anchor for `arch`: its own if measured, else the nearest measured generation."""
  key = arch.strip().lower()
  if key in _ANCHORS: return key
  from . import _targets as _TG
  fam = int(_TG.family_of_arch(key))
  if fam >= 5: return "h17s"
  if fam >= 3: return "h14"                 # A14 measured; A15 nearest below A16
  return "h13"
...
def _analytic_constants(arch: str) -> dict:
  """The analytic-roofline constants for `arch`, scaled from the nearest anchor (BW by cores, floor by clock, peak by cores*eff)."""
...
# op name -> bridge family in the JSON.
_OP_TO_FAMILY = {
  "sdpa": "bridge_sdpa",
...
# per-family "work scalar": a monotone proxy for per-call cost.
# e.g. sdpa ~ H*S^2*D (attention MACs); argmax/topk/sort ~ C*W (scan).
_BRIDGE_WORK = {
...
def _parse_bridge_key(family: str, key: str):
  """Parse a bridge_ops key's size string into a number tuple, per family; None on mismatch."""
...
def _bridge_model() -> dict:
  """model.bridge_ops parsed into {family: [(size_tuple, min_us), ...]}."""
  out: dict = {}
  path = _cost_model_path()
...
def _node_size(fam: str, t):
  """A bridge node's size tuple in the same convention _parse_bridge_key produces; None on shape mismatch."""
  s = t.srcs
...
def bridge_cost(t):
  """Measured per-call cost (us) for a bridge node (nearest-neighbour by work scalar), or None if no data."""
  ...
  # nearest measured point by work scalar (log space for symmetric ratios)
...
def _weight_elems(t) -> int:
  """Total constant-weight elements a node carries (any ndarray attr counts as streamed weight)."""
...
def _node_roofline_us(t, c: dict, int8: bool) -> float:
  """max(compute, memory) microseconds for one node (NO floor; overhead is charged once per program)."""
  in_elems = sum(_elems(s.shape) for s in t.srcs)
  ...
  flops = _node_flops(t)
  fpus = c["flops_per_us"]
  uf = c.get("util_k")                   # mid-utilization compute ramp (h14), capped at peak
  if uf and flops > 0:
...
def _estimate_analytic(out, arch: str, int8: bool) -> float:
  """Measurement-free per-chip latency (us): overhead + sum_nodes max(compute, memory) per program, + a round-trip per cut."""
...
_NARROW_SUM_FLOOR = 256      # contraction length above which a signed reduce_sum
                             # loses digits to the narrow accumulator


def _is_signed_producer(t) -> bool:
  """True if `t` can produce signed values (non-negative outputs excluded, else conservatively signed)."""
