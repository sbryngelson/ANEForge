...
`ane_rank_fused.py` provides four hardware-native fp16 rank layers via netplists: `Sort`, `TopK`, `ArgMinMax`, `GlobalArgMinMax`. Input `x` is fp16 `[channels, width]` (height=1) for Sort/TopK/GlobalArgMinMax, or `[channels, height, width]` for ArgMinMax.
...
3. Only the first `KernelChannel` output channels are normalized; the rest are identity-copied. Use `KernelChannel = C` for full coverage (`C==KernelChannel` works, e.g. `C=5/KC=5`).

The window is a local channel window of size `N = C`, asymmetric-centered on `c` and clipped at boundaries: `window(c) = [max(0, c-(N-1)//2) : min(C, c + N//2 + 1)]`. Only the center channel sees all C channels; edge channels see a partial sum, not a full-channel sum. Arch-gated to `C <= 15` (`C >= 16` fails ANECCompile).

!!! note "Two LRN paths"
    `af.local_response_norm` is the fused MIL op (no cut) - `x / (k + alpha/size * sum_{window} x2)  beta` over `size` neighbouring channels. `af.lrn` is this netplist bridge (graph cut) with the corrected window semantics above.
...
`y = scale * (x OP z)` - fuses a binary elementwise op with a scalar scale. `Params.Type` selects the op (Add/Mult/Min/Max); `Params.Scale` is an fp16 bit pattern (use `fp16_bits`). Two arch quirks are guarded: `Sub` is rejected by ANECCompile, and `Mult` ignores `scale` on-silicon - both rejected rather than emitting a wrong or uncompilable program.

## einsum (native single-equation layer)

The native MIL `einsum` op (distinct from the general `af.einsum` decomposer) is a single hardware layer. The only on-ANE-verified equation is `'nchw,nwhu->nchu'` - a batched matmul over the W/U dims sharing N,H: `a`=`[N,C,H,W]`, `b`=`[N,W,H,U]` (streamed weight) -> `[N,C,H,U]`. `b` is a streamed weight array, not a graph Tensor.
