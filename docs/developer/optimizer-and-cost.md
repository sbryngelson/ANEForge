...
`_cost.py` is an op-agnostic structural cost model - the optimizer's pruner. `estimate(out) -> microseconds` gives a fast roofline estimate of a compiled program's latency without touching the device.
...
| `CUT_US` | per-cut penalty (native sub-program + host round-trip) | composition probe ~150-300 us; 200 documented mid |
| `BW_BPUS` | effective fp16 streaming bandwidth | matmul sweep: K=4096 streams 32 MiB in ~293 us -> ~114 GB/s ~ 1.1e5 B/us |
| `FLOPS_PUS` | effective fp16 compute | conv C=512: 4.83 GFLOP in ~334 us -> ~14.5 TFLOP/s ~ 1.45e7 flop/us |
...
The compiler carries its own analytic cycles->roofline->wall-time model (ZinNEPerf, non-SIP). It was decompiled and the per-chip HAL perf fields + freq/efficiency curves walked live for all 28 targets into the bundled `costmodel_curves.json`. The model:
...
- A14/h14 (M2 Pro) - measured. Carries a mid-utilization compute ramp `eff_peak = peak * min(1, (flops/F)^q)` (a capped power law, F=1.8e10 FLOP, q=0.38). Effective compute throughput is far below peak for mid-size ops (a 768^3 GEMM sustains ~1.5 of 7.24 TFLOP/s) and ramps with per-op FLOPs; the cap keeps large ops (a 2048^3 GEMM already at peak) from being slowed. Cuts the h14 25-point grid's mean error 1.61x -> 1.16x.
- A16/h17s (M5) - the 2026-06-05 loop-closure re-fit: BW 57 GB/s, floor ~110 us, peak 8.9 TFLOP/s (= `project_peak('h17s')`, validated by the re-fit landing the quoted convs within ~13%).
...
- BW scales with CORE COUNT, not clock - the verified M5 loop-closure mechanism. The earlier single-anchor model scaled M1's BW by clock and over-predicted M5 ~2x (mean |err| 99%); effective BW tracks cores (16/4 -> 5.5x), because a faster clock doesn't widen the DMA path - more cores do.
