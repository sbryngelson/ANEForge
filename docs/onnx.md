...
- fp16 compute. ANEForge casts weights to fp16 at compile and computes in fp16, so the
...
- fp16 input. `af.input` is fp16, so feed an fp16 array (cast with
  `x.astype(np.float16)`).
...
- Uniform/symmetric conv params. Per-axis strides, dilations, and pads must be
  uniform and symmetric; a non-uniform value raises rather than mis-lowering.
...
- Shape subgraph. `Shape` and the dynamic-reshape plumbing (`Slice`/`Gather`/`Concat`/
  ...
  static ops. `Slice` on an activation lowers to `slice_by_size` (step 1 only).
...
- Elementwise. `Add`/`Sub`/`Mul`/`Div`/`Pow` accept either two tensors or a tensor
  ...
  This makes `HardSigmoid`/`HardSwish` and the scale/shift ops in MobileNetV3 and
  ConvNeXt importable.
...
- Boolean algebra has no ANE path. `And`/`Or`/`Xor` cannot be implemented: MIL's
  ...
  datapath, so `x != x` reads false on-device. Comparisons and `Not` are the supported
  boolean surface.
...
- Resize: `[N,C,H,W]`, opset 11+, target from `sizes` (preferred) or `scales`. Only
  the coordinate conventions the ANE matches are accepted, each validated on-device
  (cosine ~1.0); every other config raises rather than mis-resize:
    ...
    - `half_pixel`/`pytorch_half_pixel` sampling and `mode="cubic"` are not matched by
      the ANE resamplers and raise. The ONNX default `coordinate_transformation_mode`
      ...
      ANE bilinear (an opt-in approximation, ~0.99 cosine on smooth maps). This lets
...
- Gather: static (constant-initializer) integer indices only - a data-dependent
  ...
  raises. Lowers to `slice_by_size`+`concat`; validated on-device (cosine ~1.0).
...
- Attention (opset 23): maps onto `af.sdpa`, the native fused-attention layer, so it
