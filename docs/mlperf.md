...
accuracy. Believed to be the first MLPerf-shaped ResNet-50 result on the ANE. Unofficial: self-measured, no
MLCommons audit trail.
...
aneforge compiles the ONNX graph to one fused e5rt program and runs it under the ANE device mask. No CPU/GPU
fallback: an unsupported op fails the compile. SingleStream p90 is 0.773 ms, 70x the same graph on CPU.
...
fp16 datapath on large values. Folding `Conv -> BatchNorm` into the conv (exact) removes it, now done
automatically by the ONNX importer. Full 50k ILSVRC-2012 val, MLPerf preprocessing:
...
`bench/mlperf/compare/compare.py` places these next to official MLPerf results, each sourced to an
...
is the ResNet-50 ceiling on the engine.
...
MLCommons audit trail, not submitted in an official round -- that is a membership and process step.
